...
**自治边界**：ESP32 保留传感器、执行器、本地规则和安全护栏，断网断树莓派也能种活作物。树莓派承担联网、大屏、AI——挂了只是变笨/看不到。大屏只读，不向 ESP32 下发控制命令。AI（DeepSeek）跑在树莓派侧（见 §4 两层降级、§9.4）。
...
> 这些随 `ai_client.py` 一起删除。下面的 4.1 改为"两层降级"。

### 4.1 两层降级架构

决策从聪明到可靠分两层，上层挂了下层接住（2026-05-31 移除了多余的树莓派阈值规则中间层）：
...
② ESP32 本地规则（utils.local_fallback_decision，板上常驻兜底）
        ↑ DeepSeek 失败、或 UART 断 / 树莓派没了，都回到这层
...
传感器故障 → 自动切安全值 → 继续运行
执行器运行中 → 跳过新动作 → 避免叠加
DeepSeek 超时/失败 → 网关不发 advice → ESP32 本地规则兜底
...
- **`--ai-advice`**：收到 report 后调 DeepSeek（经 9.4 的 `pi_advisor`）→ advice 回 ESP32；**AI 失败则不发 advice**，ESP32 回到本地规则（2026-05-31 移除了树莓派阈值规则中间层）
