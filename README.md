...
> 基于 ESP32 + 云端 AI 的全自动植物养护 IoT 系统 | 14 种作物库（拨码现场切换 8 种）· 双引擎决策 · 四级测试体系
...
| 👨‍🚀 人力昂贵 | **全自动养护闭环** | 感知→决策→执行全链路自动化，拨码一键切换 8 种作物（库内 14 种参数），无需人工配置 |
...
系统以 **ESP32 为下位机**，通过 **4 类传感器**实时感知环境，借助 **云端 DeepSeek 大模型 + 本地规则引擎**双重决策，驱动 **水泵和营养液泵**自动养护作物（库内 14 种参数，3 位拨码现场切换 8 种核心作物）。从传感器到执行器、从 OLED 小屏到 Web 大屏（模拟地面遥测站），**全链路自主设计、全代码自主编写**。
...
| 🌱 **14 种作物生长阶段模型** | 拨码开关现场切换 8 种核心作物（库内共 14 种） | 宇航员无需任何农业知识——一键切换，系统自动匹配从苗期到采收期的完整水肥策略 |
...
| 准备比赛答辩（含 A/B 双套立意） | [deliverables/评委展示方案.md](./deliverables/评委展示方案.md) |
| 5 年级专用：A 套立意背诵手册 | [deliverables/育种叙事-背诵手册.md](./deliverables/育种叙事-背诵手册.md) |
...
每一项工程指标都可以从两个视角读：**"宇航员菜园"** 与 **"在轨育种验证平台"**。

| 指标 | 数值 | 菜园视角 | 育种平台视角 |
|:-----|:-----|:---------|:-------------|
| 自动化测试 | **74 个用例 ALL PASS** | 故障预案演练 | 科研级数据可靠性保证 |
| 支持作物 | **库内 14 种** / 现场拨码 8 种 | 多品种轮种 | **多品种平行筛选能力** |
| 生长阶段模型 | 每作物 **3-5 个阶段** | 全程自动化 | **全生长周期数据闭环** |
| 容错能力 | 传感器离线降级 + 看门狗 + 动作限频 | 系统永不离线 | 长周期育种实验不被中断 |
| 决策延迟 | 云端 AI < 3s，本地规则 < 1ms | 满足深空延迟 | 实时筛选有价值的突变性状 |
| 采样周期 | **60 秒**采样一次 | 节流省电 | **高密度生长数据采集** |
| 固件模块 | **13 个**，单文件最大约 300 行 | 模块化维护 | 在轨远程热更新维护 |
| 硬件成本 | **¥135/套** | 教育普及 | 低成本批量部署，覆盖更多品种 |
| 实机运行 | 超过 **18 天**连续运行 | 7×24 无人值守稳定性 | 跑通一个完整速生菜生长周期 |
