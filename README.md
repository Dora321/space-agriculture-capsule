...
在月球基地、火星前哨或长期空间站任务中，新鲜蔬菜不仅是营养补给，更是宇航员心理健康的支柱。但"在太空种菜"面临的挑战与地面温室**完全不同**：
...
| 指标 | 数值 | 太空意义 |
|:-----|:-----|:---------|
| 自动化测试 | **74 个用例 ALL PASS** | 包含断网、传感器失效、执行器降级、温度安全护栏等故障预案 |
| 支持作物 | **库内 14 种** / 现场拨码 8 种（叶菜 4 + 果菜 4） | 覆盖从 30 天速生菜到 150 天长周期果菜 |
| 生长阶段模型 | 每种作物 **3-5 个阶段** | 苗期→营养生长期→花期→果期→采收期，全程自动化 |
| 容错能力 | **传感器离线降级 + 看门狗 + 动作限频** | 模拟在轨无人维修场景，确保系统永不离线 |
| 决策延迟 | 云端 AI < 3s，本地规则 < 1ms | 满足火星 4-24min 延迟下的全自治要求 |
| 固件模块 | **13 个**，单文件最大约 300 行 | 模块化便于在轨远程热更新维护 |
| 硬件成本 | **¥135/套** | 低成本原型，适合 STEM 课程批量部署 |
| 实机运行 | 超过 **18 天**连续运行 | 验证 7×24 无人值守长期稳定性 |
