...
> **2026-05-27 注**：本文档为设计阶段方案，下文中 `nutrient_sec` / `nutrient_interval` / `last_nutrient` 等字段已在单泵架构下移除；动作集合现仅为 `{water, idle}`。本文档作为 OLED 显示设计的历史决策记录保留，请以当前 `display.py` 实现为准。
...
- **页码指示**: 底部 3 个 3x3 像素方点，当前页实心；不使用 `● ○ ○` 等 Unicode 字符，避免 SH1106 内置 ASCII 字体无法显示
...
- **当前硬件无补光灯**，只有 12V 水泵（2026-05-27 起单泵架构，已无营养液泵）
- 补光灯需要额外 GPIO + 继电器通道
- 设计为**提示用户**移动种植舱位置
...
- 但要注意: SH1106 的 `fill_rect` 在 MicroPython 中性能OK

### 5.5 页码为什么用像素点而不是字符圆点？

- SH1106 MicroPython 内置 `text()` 字体主要覆盖 ASCII，`●/○` 这类字符可能显示为空白或乱码
